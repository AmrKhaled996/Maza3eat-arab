...
import { PostformatDate } from "../../utils/DateFormater";
  ...
  return (
    ...
    >
      ...
      <div className="relative sm:w-52 md:w-80 shrink-0 overflow-hidden group-hover:scale-103 transition-transform duration-700 ease-in-out">
        ...
        />
        {post?.image?.remainingImages && post.image.remainingImages > 0 && (
          <span className="absolute w-8 h-8 top-3 right-3 bg-black/50 text-white text-xs font-bold px-2 py-0.5 rounded-full backdrop-blur-sm flex items-center justify-center">
           {post.image.remainingImages}<Plus size={16} className="mt-0.5"/> 
          </span>
        )}
      </div>

      {/* Content */}
      <div className="p-5 flex flex-col flex-1 group-hover:bg-black/1" dir="rtl">
        {/* Author */}
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          <span className="mr-auto text-xs text-gray-400">{ post?.publishDate ? PostformatDate(post.publishDate) : '' }</span>
        </div>
